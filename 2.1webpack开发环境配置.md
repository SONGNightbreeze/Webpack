...
# 总体流程: 首先通过entry将 index.js 人口文件加载进来，entry会分析内部的依赖
# 发现要引入 index.css资源，所以现在要引入的是两个资源 index.js和index.css
# 资源会经过rules里面的loader进行处理，匹配文件 .css然后从下向上执行use里的loader
# 先执行'css-loader' 将index.css资源以commonjs形式整合到.js文件中，
# 所以检查built.js资源的时候会发现里面有样式资源并且以字符串的形式体现
# 在页面中打开的时候执行'style-loader' 会创建style标签，插入到页面中生效
# 最后的输出路径就是有output来决定
# 所以对于webpack要处理一些资源就要添加响应的loader
